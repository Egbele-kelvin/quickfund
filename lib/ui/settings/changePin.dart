...
class _ChangePinState extends State<ChangePin> {
  ...
  void resetPin(
      ResetPin resetPin, SettingsProvider settingsProvider) async {
    setState(() {
      isLoading = true;
    });
    try {
      await settingsProvider.resetPin(resetPin);
                    ...
                    isLoading
                      ...
                      onTap: () {
                        if (_formKey.currentState.validate()) {
                          _formKey.currentState.save();
                          KeyboardUtil.hideKeyboard(context);
                          var resetPasswordParams = ResetPin(
                              phone: phoneNumber,
                              otp: _pin,
                              currentPin: oldPassword,
                              pin: password,
                              pinConfirmation: confirmPassword
                          );
                          resetPin(resetPasswordParams, settingProvider);
                        }
                      },
                      btnTitle: 'Continue',
                    ),
